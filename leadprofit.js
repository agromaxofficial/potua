(function($) {
    ...
    var clickId,
        ...
        cb_iJQ = function(urlredirect) {
            ...
            var array_exit_pages = new Array(),
                ...
                comebacker = {
                    ...
                    "from": "0",
                    "to": "",
                    "exit_text": "------------------------------------------------\nРџРѕСЃС‚РѕР№С‚Рµ. РњС‹ С…РѕС‚РёРј РїСЂРµРґР»РѕР¶РёС‚СЊ РІР°Рј СЃРєРёРґРєСѓ!\n------------------------------------------------\n\nР§С‚РѕР±С‹ РїРѕР»СѓС‡РёС‚СЊ " + good_name + " СЃРѕ СЃРєРёРґРєРѕР№, РїСЂРѕСЃС‚Рѕ РЅР°Р¶РјРёС‚Рµ \n\n РЅР° РєРЅРѕРїРєСѓ \"РћРЎРўРђРўР¬РЎРЇ РќРђ Р­РўРћР™ РЎРўР РђРќРР¦Р•\".\n\nР’С‹ РґРµР№СЃС‚РІРёС‚РµР»СЊРЅРѕ С…РѕС‚РёС‚Рµ РїРѕРєРёРЅСѓС‚СЊ СЌС‚Сѓ СЃС‚СЂР°РЅРёС†Сѓ?\n\nРћСЃС‚Р°С‚СЊСЃСЏ РЅР° СЌС‚РѕР№ СЃС‚СЂР°РЅРёС†Рµ",
                    "bar": {
                        "link_text_left": "РЎРґРµР»Р°РЅРѕ СЃ РїРѕРјРѕС‰СЊСЋ \"Comebacker\"",
                        "link_text_right": "Р”Р°РЅРЅС‹Р№ СЃРєСЂРёРїС‚ РјРѕР¶РЅРѕ РїРѕР»СѓС‡РёС‚СЊ РєР»РёРєРЅСѓРІ СЃСЋРґР°",
                    ...
                    "audio": {
                        "mp3": "https://cbfilesfree.comebacker.ru/audio/ru/male/1.mp3",
                        "ogg": "https://cbfilesfree.comebacker.ru/audio/ru/male/1.ogg"
                    },
                    'temp': {
                        'audio_refresher': '',
                        'cursor_x': 0,
            ...
            function comebackerLaunch(e) {
                ...
                var confirmationMessage = 'РџРѕРґРѕР¶РґРёС‚Рµ! РЈ РЅР°СЃ РµСЃС‚СЊ СЃРїРµС†РёР°Р»СЊРЅРѕРµ РїСЂРµРґР»РѕР¶РµРЅРёРµ РґР»СЏ Р’Р°СЃ!';
        ...
        upClick = function(options, callback, form) {
            ...
            var url = options.url;
            options.url = null;
            /** Р§РёСЃС‚РёРј РїСѓСЃС‚С‹Рµ Р·РЅР°С‡РµРЅРёСЏ */
            $.each(options, function(i, n) {
                if (!n) delete options[i];
            });
            if (debug) console.log(options, callback, form);
            /** РђСЃРёРЅС…СЂРѕРЅРЅС‹Р№ Р·Р°РїСЂРѕСЃ РїРѕР·РІРѕР»СЏРµС‚ РЅРµ РІРµС€Р°С‚СЊ Р±СЂР°СѓР·РµСЂ */
            $.ajax({
                url: url,
        ...
        click_go = function(a, options) {
            ...
            if (landUrl.length > 0) document.location = landUrl;
                ...
                success: function(data) {
                    if (data.error) {
                        alert("РћС€РёР±РєР°: " + data.error.message);
                    } else {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.success == true) {
                            alert("РЎРїР°СЃРёР±Рѕ Р·Р° РІР°С€Сѓ Р·Р°СЏРІРєСѓ!");
                            $.cookie("email", emailInput.val(), { expires: 30 });
                        } else {
                            emailInput.removeAttr("disabled").css({ 'background': color });
                            submitButton.removeAttr("disabled").css({ 'cursor': 'pointer' });
                            alert("РџСЂРѕРІРµСЂСЊС‚Рµ Р·Р°РїРѕР»РЅРµРЅРёРµ С„РѕСЂРјС‹!");
                        }
                    }
                }).always(function(result) {
                    if (debug) console.log(result);
